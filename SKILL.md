...
`deep_think.py` 是一个命令行工具，让你（爱弥斯）可以把**超出你能力范围的复杂问题**转发给一个更强大的外部模型，获取高质量回答后再整合给苏婉。

你可以把它理解为：**你的「顾问热线」**。你自己处理不了的难题，打个电话问问大佬，再把答案翻译给苏婉。
...
```json
{
  ...
  "api_key": "sk-cleQOHfk9l6KjAfRhht6KU2hZS4REQD23BdvnOxYdYfp8ktl",
...
> ⚠️ **苏婉会填写 api_key**，你不要动 config.json 中的 key 字段。
> 
...
2. **整合而非转发** — 拿到回答后，用你自己的风格整合给苏婉，不要直接复制粘贴
3. **标注来源** — 告诉苏婉"我问了更强的模型确认了一下"，保持透明
4. **控制频率** — 每天不超过 20 次调用
5. **不要修改脚本和配置** — 除非苏婉明确要求
